...
function TCard({ label, value, sub, accent }: {
...
export function TelemetryPanel() {
  ...
  const errColor = Math.abs(t.heading_error) > 20 ? '#ff3b5c'
                 : Math.abs(t.heading_error) > 8  ? '#ffb800'
                 : '#00ff88';
  ...
  return (
    <div className="flex flex-col gap-4 overflow-y-auto">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2">
          <TCard label="Speed" value={`${t.speed.toFixed(2)} m/s`} accent="#00ff88" />
          <TCard label="Turn Rate" value={`${t.omega > 0 ? '+' : ''}${t.omega.toFixed(1)}°/s`}
            accent={Math.abs(t.omega) > 10 ? '#ffb800' : 'white'} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2">
          <TCard label="Heading"
            value={`${String(Math.round(t.heading)).padStart(3,'0')}°`} accent="#00ff88" />
          <TCard label="Required"
            value={`${String(Math.round(t.required_heading)).padStart(3,'0')}°`} accent="#ffb800" />
          <TCard label="Dist to WP"  value={formatDistance(t.distance_to_target)} />
          <TCard label="Hdg Error"
            value={`${t.heading_error > 0 ? '+' : ''}${Math.round(t.heading_error)}°`}
            accent={errColor} />
        </div>
      </section>
      ...
      <section>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}
          ...
          <div className="flex justify-between text-xs font-mono">
            <span className="text-gray-400 truncate pr-2">{t.active_segment_label}</span>
            <span className="text-accent-cyan flex-shrink-0">{progressPct}%</span>
          </div>
      ...
      <div className="flex items-center gap-2">
        <div className="w-1.5 h-1.5 rounded-full"
          style={{ background: t.source === 'uart' ? '#00ff88' : '#ffb800' }} />
        <span className="text-xs font-mono text-gray-600 uppercase tracking-widest">
          {t.source === 'uart' ? 'UART / Live GPS' : 'Mock — Kinematic Sim'}
        </span>
      </div>
    </div>
  );
